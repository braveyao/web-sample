...
/* @internal */
export const computeDefaultScreenShareSimulcastPresets = (
  fromPreset: VideoPreset
) => {
  const layers = [
    { scaleResolutionDownBy: 2, fps: fromPreset.encoding.maxFramerate },
  ];
  return layers.map(
    (t) =>
      new VideoPreset(
...
export function computeVideoEncodings(
  ...
  if (
    (!videoEncoding && !useSimulcast && !scalabilityMode) ||
    !width ||
    !height
  ) {
  ...
  if (scalabilityMode && isSVCCodec(videoCodec)) {
    ...
    if (
      ...
      isReactNative() ||
      (browser?.name === "Chrome" &&
        compareVersions(browser?.version, "113") < 0)
    ) {
  ...
  if (isScreenShare) {
    ...
    presets =
      sortPresets(options?.videoSimulcastLayers) ??
      defaultSimulcastLayers(isScreenShare, original);
  }
  ...
  if (isReactNative() && getReactNativeOs() === "ios") {
    ...
    encodings.forEach((encoding) => {
      if (encoding.maxFramerate != topFramerate) {
        ...
        log.info(
          `Setting framerate of encoding \"${encoding.rid ?? ""}\" to ${topFramerate}`
        );
        encoding.maxFramerate = topFramerate;
      }
    });
...
export function sortPresets(presets: Array<VideoPreset> | undefined) {
  ...
  return presets.sort((a, b) => {
    ...
    if (
      aEnc.maxBitrate === bEnc.maxBitrate &&
      aEnc.maxFramerate &&
      bEnc.maxFramerate
    ) {
...
export class ScalabilityMode {
  ...
  constructor(scalabilityMode: string) {
    const results = scalabilityMode.match(
      /^L(\d)T(\d)(h|_KEY|_KEY_SHIFT){0,1}$/
    );
    if (!results) {
...
export function getDefaultDegradationPreference(
  track: LocalVideoTrack
  ...
  if (
    track.source === Track.Source.ScreenShare ||
    (track.constraints.height &&
      unwrapConstraint(track.constraints.height) >= 1080)
  ) {
