...
export default class Participant extends (EventEmitter as new () => TypedEmitter<ParticipantEventCallbacks>) {
  ...
  private _qualityChangeTimeout?: ReturnType<typeof setTimeout>;

  private _lastProtoQuality?: ProtoQuality;
  ...
  get joinedAt(): Date | undefined {
    if (this.participantInfo) {
      return new Date(
        Number.parseInt(this.participantInfo.joinedAt.toString()) * 1000
      );
    }
    return new Date();
  }
  ...
  setConnectionQuality(q: ProtoQuality) {
    ...
    this._qualityChangeTimeout = setTimeout(() => {
      const newQuality = qualityFromProto(this._lastProtoQuality!);
      const prevQuality = this._connectionQuality;

      if (prevQuality !== newQuality) {
        this._connectionQuality = newQuality;
        console.log(
          `Connection quality changed from ${prevQuality} to ${newQuality}`
        );
        this.emit(ParticipantEvent.ConnectionQualityChanged, newQuality);
      }

      // Clear the reference
      this._qualityChangeTimeout = undefined;
    }, 15000);
  }
    ...
    this.audioTrackPublications.forEach(
      (track) =>
        (track.track instanceof RemoteAudioTrack ||
          track.track instanceof LocalAudioTrack) &&
...
export type ParticipantEventCallbacks = {
  ...
  participantPermissionsChanged: (
    ...
    currPermissions?: ParticipantPermission
  ) => void;
