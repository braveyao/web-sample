...
export class DeepFilterLoader {
    ...
    static async loadWasmData(): Promise<void> {
        ...
        try {
            ...
            const modelResponse = await fetch(`${basePath}DeepFilterNet3_ll_onnx.tar.gz`, {
                headers: {
                    'Accept-Encoding': 'identity' // Request no compression to get raw file
                }
            });
            if (!modelResponse.ok) {
                throw new Error(`Failed to fetch model data: ${modelResponse.status} from ${basePath}DeepFilterNet3_ll_onnx.tar.gz`);
            }
