...
// Global Set to track printed final segments.
// The key is constructed as `speaker-finalText`
const printedFinalSegments = new Set<string>();
...
function updateTranscriptions(
  ...
  console.log("TranscriptionReceived RoomEvent");
  for (const segment of segments) {
   console.log("TranscriptionSegment");
   console.log(segment);
   console.log(participant);
   console.log(`[ Speaker: ${participant?.identity}, TranscriptionSegment id: ${segment.id}, text: ${segment.text}
    , language: ${segment.language}, firstReceivedTime: ${segment.firstReceivedTime}, lastReceivedTime: ${segment.lastReceivedTime}
    , startTime: ${segment.startTime}, endTime: ${segment.endTime}, final: ${segment.final}, error: ${segment.error}, failureReason: ${segment.failureReason} ]`);
  }

  const liveChatDiv = $('live-transcription');
  const finalChatDiv = $('final-transcription');
  const speaker = participant?.identity || 'Unknown';

  segments.forEach(segment => {
    // Format timestamp using date and time in 12-hour format.
    const dateObj = new Date(segment.lastReceivedTime);
    const datePart = dateObj.toLocaleDateString('en-US'); // e.g., "04/15/2025"
    const timePart = dateObj.toLocaleTimeString('en-US', { hour12: true }); // e.g., "07:13:20 PM"
    const formattedDateTime = `${datePart} ${timePart}`;

    if (segment.final) {
      // Use speaker and trimmed final text as a unique key.
      const finalKey = `${speaker}-${segment.text.trim()}`;
      // Append the final message only if it wasn't already printed.
      if (!printedFinalSegments.has(finalKey)) {
        printedFinalSegments.add(finalKey);
        ...
        // Create a span for the speaker name.
        const speakerSpan = document.createElement('span');
        speakerSpan.style.fontWeight = 'bold';
        speakerSpan.textContent = speaker + ': ';

        // Create a span for the final text.
        const textSpan = document.createElement('span');
        textSpan.style.color = 'green';
        textSpan.textContent = segment.text;

        // Append the spans to the final container.
        finalDiv.appendChild(timestampSpan);
        finalDiv.appendChild(speakerSpan);
        finalDiv.appendChild(textSpan);
        finalChatDiv.appendChild(finalDiv);
      }
      // Remove any live transcription for this speaker.
      const liveEntry = document.getElementById(`live-${speaker}`);
      if (liveEntry) {
        liveEntry.remove();
      }
    } else {
      // Handle live (partial) update.
      let liveEntry = document.getElementById(`live-${speaker}`);
      if (!liveEntry) {
        liveEntry = document.createElement('div');
        liveEntry.id = `live-${speaker}`;

        // Create and append the individual elements.
        const timestampSpan = document.createElement('span');
        timestampSpan.id = `live-timestamp-${speaker}`;
        timestampSpan.style.color = 'grey';
        timestampSpan.textContent = formattedDateTime + ' ';
        ...
        const textSpan = document.createElement('span');
        textSpan.id = `live-message-${speaker}`;
        textSpan.style.color = 'blue';
        textSpan.textContent = segment.text;

        liveEntry.appendChild(timestampSpan);
        liveEntry.appendChild(speakerSpan);
        liveEntry.appendChild(textSpan);
        liveChatDiv.appendChild(liveEntry);
      } else {
        // Update the timestamp and text spans if they exist.
        const timestampSpan = document.getElementById(`live-timestamp-${speaker}`);
        if (timestampSpan) {
          timestampSpan.textContent = formattedDateTime + ' ';
        }
        const textSpan = document.getElementById(`live-message-${speaker}`);
        if (textSpan && textSpan.textContent !== segment.text) {
          textSpan.textContent = segment.text;
        }
      }
    }
  ...
  // Auto-scroll both transcription containers.
  liveChatDiv.scrollTop = liveChatDiv.scrollHeight;
  finalChatDiv.scrollTop = finalChatDiv.scrollHeight;
}
...
const appActions = {
  ...
  changeNoiseSuppression: async () => {
    ...
    try {
      ...
      console.error('Failed to change denoise mode:', error);
    }
  },
};
...
// There is chance that audioContext is not set to audio track befre LocalTrackPublished
// event is triggered in Chrome/Edge.
/*
...
function participantConnected(participant: Participant) {
  ...
  participant
    ...
    .on(ParticipantEvent.TranscriptionReceived, (segments) => {
      console.log("TranscriptionReceived ParticipantEvent:", segments);
      segments.forEach(segment => {
        console.log(`Speaker: ${participant?.identity}, Text: ${segment.text}`);
      });
    })
