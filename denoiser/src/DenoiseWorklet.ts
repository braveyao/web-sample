...
// Declare the global registerProcessor function for AudioWorklet
...
class DenoiseWorklet extends AudioWorkletProcessor {
    ...
    /**
     * Flag to track if denoising is enabled
     */
    ...
    constructor(options: any) {
        ...
        (this as any).port.onmessage = (event: MessageEvent) => {
    ...
    private async initializeProcessor() {
        try {
            if (this.useDeepFilter) {
                console.warn("DenoiseWorklet: DeepFilter is not supported in this build");
                return;
            } else {
    ...
    process(inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
        ...
        // If denoising is disabled, pass through the audio without processing
        if (!this.enabled) {
            const inData = inputs[0]?.[0];
            const outData = outputs[0]?.[0];
